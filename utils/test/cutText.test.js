...
describe('CutText', () => {
  ...
  it('should return an error if "content" arg equals 0', () => {
    expect(cutText('', 20)).to.equal('Error');
  });

  it('should return an error if "maxLength" arg is not a number', () => {
    expect(cutText('Lorem', undefined)).to.equal('Error');
    expect(cutText('Lorem', 'Lorem')).to.equal('Error');
    expect(cutText('Lorem', {})).to.equal('Error');
    expect(cutText('Lorem', [20])).to.equal('Error');
    expect(cutText('Lorem', function() {})).to.equal('Error');
  });
